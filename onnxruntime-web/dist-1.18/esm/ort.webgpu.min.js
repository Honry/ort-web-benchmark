...
}`,ru=e=>{let t=at(e.inputs[0].dataType);e.compute(Pe(e.inputs[0],"Erf",r=>`erf_vf32(${r})`,Qn(t)))},nu=e=>{e.compute(Pe(e.inputs[0],"Exp","exp"))},iu=e=>{e.compute(Pe(e.inputs[0],"Floor","floor"))},au=e=>{let t=at(e.inputs[0].dataType);e.compute(Pe(e.inputs[0],"Gelu",r=>`0.5 * ${r} * (1.0 + erf_vf32(${r} * 0.7071067811865475))`,Qn(t)))},ou=(e,t)=>{let r=at(e.inputs[0].dataType);e.compute(Pe(e.inputs[0],"LeakyRelu",n=>`select(leaky_relu_alpha_ * ${n}, ${n}, ${n} >= vec4<${r}>(0.0))`,`const leaky_relu_alpha_ = ${r}(${t.alpha});`,t.cacheKey))},su=e=>{e.compute(Pe(e.inputs[0],"Not",t=>`!${t}`))},uu=e=>{e.compute(Pe(e.inputs[0],"Neg",t=>`-${t}`))},lu=e=>{e.compute(Pe(e.inputs[0],"Reciprocal",t=>`1.0/${t}`))},du=e=>{let t=at(e.inputs[0].dataType);e.compute(Pe(e.inputs[0],"Relu",r=>`select(vec4<${t}>(0.0), ${r}, ${r} > vec4<${t}>(0.0))`))},cu=e=>{e.compute(Pe(e.inputs[0],"Sigmoid",t=>`(1.0 / (1.0 + exp(-${t})))`))},pu=e=>we(e),mu=(e,t)=>{let r=at(e.inputs[0].dataType);e.compute(Pe(e.inputs[0],"HardSigmoid",n=>`max(vec4<${r}>(0.0), min(vec4<${r}>(1.0), ${t.alpha} * ${n} + vec4<${r}>(${t.beta})))`,void 0,t.cacheKey))},fu=e=>{e.compute(Pe(e.inputs[0],"Sin","sin"))},hu=e=>{e.compute(Pe(e.inputs[0],"Sinh","sinh"))},gu=e=>{e.compute(Pe(e.inputs[0],"Sqrt","sqrt"))},yu=e=>{e.compute(Pe(e.inputs[0],"Tan","tan"))},bu=e=>`sign(${e}) * (1 - exp(-2 * abs(${e}))) / (1 + exp(-2 * abs(${e})))`,vu=e=>{e.compute(Pe(e.inputs[0],"Tanh",bu))},oa=(e="f32")=>`
...
`,sa=e=>`(fast_gelu_a + fast_gelu_a * tanh_v(${e} * (fast_gelu_c * ${e} * ${e} + fast_gelu_b))) * ${e}`,wu=e=>{let t=at(e.inputs[0].dataType);e.compute(Pe(e.inputs[0],"FastGelu",sa,oa(t),void 0,e.inputs[0].dataType))},$u=(e,t)=>{let r=at(e.inputs[0].dataType);return e.compute(Pe(e.inputs[0],"ThresholdedRelu",n=>`select(vec4<${r}>(0.0), ${n}, ${n} > thresholded_relu_alpha_)`,`const thresholded_relu_alpha_ = vec4<${r}>(${t.alpha});`,t.cacheKey)),0},_u=e=>{e.compute(Pe(e.inputs[0],"Log","log"))}});var fp,hp,Su,Cu=j(()=>{"use strict";xe();_e();Jn();fp=e=>{if(e[0].dims.length!==3)throw new Error("input should have 3 dimensions");if(![2560,5120,10240].includes(e[0].dims[2]))throw new Error("hidden state should be 2560, 5120 or 10240");if(e[1].dims.length!==1)throw new Error("bias is expected to have 1 dimensions");if(e[0].dims[2]!==e[1].dims[0])throw new Error("last dimension of input and bias are not the same")},hp=e=>{let t=e[0].dims.slice();t[2]=t[2]/2;let r=U("input",e[0].dataType,e[0].dims,4),n=U("bias",e[0].dataType,[e[0].dims[2]],4),i=K("output",e[0].dataType,t,4),s=M.size(t)/4,o=Be(e[0].dataType);return{name:"BiasSplitGelu",getRunData:()=>({outputs:[{dims:t,dataType:e[0].dataType}],dispatchGroup:{x:Math.ceil(s/64)}}),getShaderSource:c=>`
  ...
  ${c.mainStart()}
    ...
    ${i.setByOffset("global_idx","valueLeft * geluRight")}
        ...
        ${e.guardAgainstOutOfBoundsWorkgroupSizes("uniforms.vec_size")}
        ${T}
      }`},yp=(e,t,r,n,i,s,o=r.dataType)=>{let d=!M.areEqual(r.dims,n.dims),c=r.dims,m=M.size(r.dims),u=!1,g=!1,b=[d];if(d){let $=zt.calcShape(r.dims,n.dims,!1);if(!$)throw new Error("Can't perform binary op on the given tensors");c=$,m=M.size(c);let w=M.size(r.dims)===1,x=M.size(n.dims)===1,C=r.dims.length>0&&r.dims[r.dims.length-1]%4===0,T=n.dims.length>0&&n.dims[n.dims.length-1]%4===0;b.push(w),b.push(x),b.push(C),b.push(T);let S=1;for(let E=1;E<c.length;E++){let k=r.dims[r.dims.length-E]??1,P=n.dims[n.dims.length-E]??1;if(k===P)S*=k;else break}S%4===0?(g=!0,u=!0):(w||x||C||T)&&(u=!0)}else u=!0;return b.push(u),{name:e,shaderCache:{hint:t+b.map($=>$.toString()).join("_"),inputDependencies:["rank","rank"]},getShaderSource:$=>gp($,r.dims,n.dims,c,u,d,g,i,r.dataType,n.dataType,o,s),getRunData:()=>({outputs:[{dims:c,dataType:o}],dispatchGroup:{x:Math.ceil(m/64/4)},programUniforms:[{type:12,data:Math.ceil(M.size(c)/4)},...X(r.dims,n.dims,c)]})}},Wt=(e,t,r,n,i,s)=>{e.compute(yp(t,i??"",e.inputs[0],e.inputs[1],r,n,s))},Iu=e=>{Wt(e,"Add",(t,r)=>`${t}+${r}`)},Tu=e=>{Wt(e,"Div",(t,r)=>`${t}/${r}`)},Au=e=>{Wt(e,"Equal",{scalar:(t,r)=>`u32(${t}==${r})`,vector:(t,r)=>`vec4<u32>(${t}==${r})`},void 0,void 0,9)},Eu=e=>{Wt(e,"Mul",(t,r)=>`${t}*${r}`)},ku=e=>{let t=U("input",e.inputs[0].dataType,e.inputs[0].dims).type.value;Wt(e,"Pow",{scalar:(n,i)=>`pow_custom(${n},${i})`,vector:(n,i)=>`pow_vector_custom(${n},${i})`},`
  ...
  fn mm_write(batch: i32, row : i32, colIn : i32, valueInput : ${n}) {
    ...
    if (row < uniforms.dim_a_outer && col < uniforms.dim_b_outer) {
      ...
      result[getIndexFromCoords4D(coords, vec4<i32>(uniforms.result_shape))/${i}] = value;
        ...
        ${w?rn(S,T,ae,void 0,!c,P):nn(S,T,ae,void 0,!c,P,!1,void 0,d)}`};return{name:"Conv2DTransposeMatMul",shaderCache:{hint:`${t.cacheKey};${S};${T};${w}`,inputDependencies:he},getRunData:()=>({outputs:[{dims:r,dataType:e[0].dataType}],dispatchGroup:{x:E[0],y:E[1],z:E[2]},programUniforms:Q}),getShaderSource:N}}});var Ep,ga,qu=j(()=>{"use strict";ye();Xt();xe();_e();Ep=(e,t,r,n,i,s=!1,o,d,c=!1)=>{let m=c?1:2,u=c?2:3,g=c?3:1,b=s?2:1,$=`
...
`}
      ...
      }`;return{name:"Slice",shaderCache:{hint:`${c.length}_${o.length}_${s.length}`,inputDependencies:["rank"]},getShaderSource:C,getRunData:()=>({outputs:[u],dispatchGroup:{x:Math.ceil(n/64)},programUniforms:x})}},ld=(e,t)=>{Hm(e.inputs,t);let r=Gm(e.inputs,t);e.compute(Fm(e.inputs,r),{inputs:[0]})},dd=e=>{let t=e.starts,r=e.ends,n=e.axes;return we({starts:t,ends:r,axes:n})}});var qm,jm,pd,md,fd=j(()=>{"use strict";ye();xe();Ze();_e();qm=e=>{if(!e||e.length!==1)throw new Error("Softmax op requires 1 input.")},jm=(e,t)=>{let r=e.dims,n=M.size(r),i=64,s=t.axis;if(s<0&&(s=r.length+s),s<r.length-1)throw new Error("softmax only supports last axis for now.");let o=r[s],d=n/o,c=Ue(o),m=o/c,u=(C,T)=>T===4?`max(max(${C}.x, ${C}.y), max(${C}.z, ${C}.w))`:T===2?`max(${C}.x, ${C}.y)`:T===3?`max(max(${C}.x, ${C}.y), ${C}.z)`:C,g=U("x",e.dataType,e.dims,c),b=K("result",e.dataType,e.dims,c),$=g.type.value,w=Be(e.dataType)==="f32"?`var threadMax = ${$}(-3.402823e+38f);`:`var threadMax = ${$}(-65504.0h);`,x=C=>`
